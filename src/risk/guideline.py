...
class ActionSuggestion:
    ...
    details: Optional[Dict[str, Any]] = (
        None  # e.g., {'quantity': 10, 'type': 'buy', 'ticker': 'AAPL'}
    )
...
class GuidelineResult:
    ...
    def __repr__(self):
        ...
        return (
            f"Result(guideline='{self.guideline_id}', status={status}, "
...
class INumeratorCalculator(ABC):
    ...
    @abstractmethod
    def calculate(
        self, raw_data: Dict[str, Any], params: Dict[str, Any]
...
class IValueSuggester(ABC):
    ...
    def calculate_suggestion(
        self, details: CheckDetails, raw_data: Dict[str, Any], params: Dict[str, Any]
    ) -> ActionSuggestion:
        """
...
class IChecker(ABC):
    ...
    @abstractmethod
    def check(
        self, guideline_params: Dict[str, Any], value_to_check: Optional[Any]
    ) -> CheckDetails:
        """
...
def _get_value(
    data: Dict, key: str, expected_type: type = float
) -> Tuple[Optional[Any], Optional[str]]:
    ...
    try:
        ...
        return (
            None,
            f"Invalid type for key {key}. Expected {expected_type.__name__}, got {type(val).__name__}. Error: {e}",
        )
...
class TotalPortfolioValueCalculator(INumeratorCalculator, IDenominatorCalculator):
    ...
    def calculate(
        ...
        if not isinstance(holdings, list):
            return (
                None,
                f"'{self.holdings_key}' key not found or not a list in raw_data",
            )
        ...
        for holding in holdings:
            if not isinstance(holding, dict):
                calculation_errors.append(
                    f"Item in '{self.holdings_key}' is not a dictionary: {holding}"
                )
            ...
            if not ticker:
                calculation_errors.append(
                    f"Missing '{self.ticker_field}' in holding: {holding}"
                )
            ...
            if qty_err:
                calculation_errors.append(qty_err)
                continue
            if price_err:
                calculation_errors.append(price_err)
                continue
    ...
    def get_required_data_keys(self, params: Dict[str, Any]) -> List[str]:
        ...
        print(
            f"WARN: Dynamic keys required for {self.__class__.__name__}. Returning generic keys based on common pattern."
        )
        # Example: Return the holdings key itself. Specific item keys are derived dynamically.
        return [
            self.holdings_key
        ]  # Caller needs to ensure individual keys exist based on holdings
...
class SingleStockValueCalculator(INumeratorCalculator):
    ...
    def calculate(
        ...
        if qty is None or price is None:
            ...
            if price is None:
                return None, f"Missing price for {ticker}"
            if qty is None:
                return None, f"Missing quantity for {ticker}"  # Or return 0?

        # Ensure qty and price are numbers before multiplying
        if not isinstance(qty, (int, float)) or not isinstance(price, (int, float)):
            return (
                None,
                f"Quantity or Price for {ticker} is not a number after retrieval.",
            )
...
class AssetClassValueCalculator(INumeratorCalculator):
    ...
    def calculate(
        ...
        if not target_class or not isinstance(target_class, str):
            return (
                None,
                "Missing or invalid 'target_identifier' (asset_class) in params",
            )
        ...
        for holding in holdings:
            ...
            if class_err:
                calculation_errors.append(
                    f"Cannot determine class for {ticker}: {class_err}"
                )
            ...
            if qty_err:
                calculation_errors.append(f"{ticker}: {qty_err}")
                continue
            if price_err:
                calculation_errors.append(f"{ticker}: {price_err}")
                continue
            if qty is None or price is None:
                calculation_errors.append(
                    f"Missing quantity or price for {ticker} in target class {target_class}"
                )
                continue

            if not isinstance(qty, (int, float)) or not isinstance(price, (int, float)):
                calculation_errors.append(
                    f"Quantity or Price for {ticker} is not a number."
                )
                continue
        ...
        if calculation_errors:
            error_summary = "; ".join(calculation_errors)
            print(
                f"Warning during AssetClassValue calculation for '{target_class}': {error_summary}"
            )
    ...
    def get_required_data_keys(self, params: Dict[str, Any]) -> List[str]:
        ...
        print(
            f"WARN: Dynamic keys required for {self.__class__.__name__}. Returning generic key."
        )
...
class RatioValueSuggester(IValueSuggester):
    ...
    ):  # Precision for suggestion quantity
    ...
    def calculate_value(
        ...
        if not isinstance(num, (int, float)) or not isinstance(den, (int, float)):
            return (
                None,
                f"Numerator or Denominator is not a number ({type(num)}, {type(den)})",
            )
    ...
    def calculate_suggestion(
        ...
        if details.error_message:
            ...
            return ActionSuggestion(
                f"Suggestion not possible due to check error: {details.error_message}",
                severity="Error",
            )
        ...
        if actual is None:
            ...
            return ActionSuggestion(
                f"{message}. Cannot determine suggestion due to missing actual value.",
                severity="Error",
            )
        ...
        if details.is_breached:
            ...
            if not isinstance(lower, (int, float)) or not isinstance(
                ...
                required_change_pct = (
                    upper - actual
                )  # Difference to bring it back *to* the limit
                action_type = "sell"
        ...
        price, price_err = (
            None,
            "Price key for suggestion not specified by numerator calculator",
        )
        if price_key:
            price, price_err = _get_value(raw_data, price_key, float)
            if price_err:
                price = None  # Nullify price if error getting it

        if (
            action_type not in ["hold"]
            ...
            try:
                ...
                if suggested_quantity_raw > 0:  # Buy or Adjust Buy
                    suggested_quantity = (
                        math.ceil(suggested_quantity_raw * scale) / scale
                    )
                elif suggested_quantity_raw < 0:  # Sell or Adjust Sell
                    suggested_quantity = (
                        math.floor(suggested_quantity_raw * scale) / scale
                    )
                else:
                ...
                if abs(suggested_quantity) < 1e-9 and abs(required_change_pct) > 1e-9:
                    ...
                    suggested_quantity = math.copysign(
                        1 / scale if scale > 0 else 1, suggested_quantity_raw
                    )
                ...
                if abs(suggested_quantity) > 1e-9:
                    ...
                    trade_details = {
                        ...
                        "type": trade_verb,
                        "ticker": params.get("target_identifier")
                        if isinstance(self.numerator_calc, SingleStockValueCalculator)
                        else None,  # Add ticker if relevant
                        "value_change_est": suggested_quantity
                        * price,  # Estimated value
                    }
                    trade_details = {
                        k: v for k, v in trade_details.items() if v is not None
                    }  # Clean None values
                else:
            ...
            if denominator is not None:
                try:
                    ...
                    trade_details = {
                        "type": action_type.replace("adjust_", ""),
                        "value_change_est": required_value_change,
                    }
                except (TypeError, ValueError, OverflowError) as e:
                ...
                trade_details = {
                    "type": action_type.replace("adjust_", ""),
                    "error": "Denominator missing",
                }

        return ActionSuggestion(
            message, severity=severity, details=trade_details if trade_details else None
        )
...
class FloatRangeChecker(IChecker):
    ...
    def check(
        ...
        target = guideline_params.get(
            "target"
        )  # Target is for info/suggestion, not check itself
        ...
        if lower is None or upper is None:
            error_msg = (
                "Guideline configuration error: Missing lower_limit or upper_limit"
            )
        ...
        if target is not None and not isinstance(target, (int, float)):
            ...
            print(
                f"Warning: Guideline target ({target}) is not a number ({type(target).__name__})"
            )
        ...
        if not is_breached:  # Only check value if params are valid so far
            if actual is None:
                ...
                error_msg = (
                    f"Value to check is not a number (type: {type(actual).__name__})"
                )
                is_breached = True
            else:
...
class Guideline:
    ...
    def check(self, data: Dict[str, Any]) -> GuidelineResult:
        ...
        try:
            ...
            value_to_check, calc_error_msg = self.value_suggester.calculate_value(
                data, self.params
            )
            ...
            suggestion = self.value_suggester.calculate_suggestion(
                details, data, self.params
            )
            ...
            if details is None:
                details = CheckDetails(
                    is_breached=True,
                    actual_value=value_to_check,
                    target_value=self.params.get("target"),
                    lower_limit=self.params.get("lower_limit"),
                    upper_limit=self.params.get("upper_limit"),
                    error_message=overall_error_msg,
                )
            # Ensure suggestion reflects the critical error
            suggestion = ActionSuggestion(
                f"Guideline processing failed: {overall_error_msg}", severity="Error"
            )
        ...
        if details is None:
            ...
            details = CheckDetails(
                ...
                upper_limit=self.params.get("upper_limit"),
                error_message=overall_error_msg
                or "Unknown error before check details generation",
            )
            if suggestion is None:
                suggestion = ActionSuggestion(
                    "Guideline processing failed critically", severity="Error"
                )

        final_error_msg = (
            details.error_message or overall_error_msg
        )  # Prioritize specific error
...
raw_portfolio_data = {
    ...
    "AAPL:qty": 10.0,
    "AAPL:price": 175.0,
    "AAPL:class": "US Equity",
    "MSFT:qty": 5.0,
    "MSFT:price": 300.0,
    "MSFT:class": "US Equity",
    "GOOG:qty": 8.0,
    "GOOG:price": 140.0,
    "GOOG:class": "US Equity",
    "TBond:qty": 20.0,
    "TBond:price": 98.0,
    "TBond:class": "US Bond",
...
for g in all_guidelines:
    ...
    print(
        f"  Result: {result.is_breached} ({'BREACHED' if result.is_breached else 'OK'})"
    )
    print(f"  Actual Value: {result.actual_value}")
    print(
        f"  Limits: [{result.lower_limit}, {result.upper_limit}], Target: {result.target_value}"
    )
